...
from modeling import EC_Predictor, transfer_iv_model
...
def build_model(tar_ec, config_dic):
    ...
    if tar_ec != "EC_1d":
        ...
        ec_predictor = transfer_iv_model(
            layer_1st[0],
            layer_1st[1],
            layer_2nd,
...
# generate run
def run_generate(tar_ec, inps, config_base, weight_base, batch_size=128):
    ...
    # predict
    for inp in inps:
        output = inp.replace(".tfrecord", "_iv.pkl")
        ...
        # predict
        names = []
        interlayer_vecs = tf.TensorArray(tf.float32, size=0, dynamic_size=True)
        from_ = 0
        for inp, name in ds:
            batch_num = tf.shape(inp)[0]
            padding_mask = create_padding_mask(inp, 0)
            _, i_vec = ec_predictor(inp, False, padding_mask)

            to_ = from_ + batch_num
            interlayer_vecs = interlayer_vecs.scatter(tf.range(from_, to_), i_vec)
            from_ = to_
        ...
        # データフレームの作成
        interlayer_vecs = interlayer_vecs.stack().numpy()

        cols = [f"v{idx}" for idx in range(1, interlayer_vecs.shape[1]+1)]
        df = pd.DataFrame(interlayer_vecs, columns=cols)
        df.loc[:, "name"] = names
        df = df.loc[:, ["name"] + cols]
        df.to_pickle(output)
